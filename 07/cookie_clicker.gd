...
func _ready() -> void:
	for building_index in CookieData.building_settings.keys().size():
		...
		var icon = load("res://07/icon/%d.png" % building_index)
		...
		if building_type == "Chancemaker":
			building.on_mini_game_pressed.connect(entrt_1_8192)
		if building_type == "Factory":
			building.on_mini_game_pressed.connect(entrt_quiz)
		...
		for tier in range(1, building_setting[2].size() + 1):
			...
			tiered_upgrade.init(building_type, tier, icon, x, y)
